...
import { getEnhancementCredits } from "../utils/credits";
import {
  ToolType,
  BackgroundMode,
  CameraAngle,
...
export const analyzeImage = async (req: AuthRequest, res: Response) => {
  try {
    const { base64, toolType = ToolType.FOOD }: AnalyzeImageRequest = req.body;
        ...
        responseSchema: {
          ...
          properties: {
            details: {
              type: "STRING",
              description:
                "Comma separated list of key elements to preserve (ingredients or product text/materials)",
            },
            props: {
              type: "ARRAY",
              items: { type: "STRING" },
    ...
    const result: AnalyzeImageResponse = text
      ? JSON.parse(text)
      : { details: "", props: [] };
 ...
 */
export const analyzeReference = async (req: AuthRequest, res: Response) => {
  try {
    const { referenceBase64 }: AnalyzeReferenceRequest = req.body;
    ...
    const result: AnalyzeReferenceResponse = { props: data.props || [] };
 ...
 */
const enhanceStyleDescription = async (
 ...
 */
export const enhanceImage = async (req: AuthRequest, res: Response) => {
  let imageTask: ImageTask | null = null;

  try {
    const { sourceBase64, styleInput, options }: EnhanceImageRequest = req.body;
    ...
    let requiredCredits = getEnhancementCredits(styleInput, options.quality);
    ...
    imageTask = await ImageTask.create({
      userId: user.id,
    ...
    const isFood = options.toolType === ToolType.FOOD;
  ...
  `;
    ...
    if (options.cameraAngle) {
      const angleMap = {
        ...
        [CameraAngle.MACRO]:
          "Macro Close-up. Shallow depth of field. Focus on texture/material details.",
...
4. EXCLUSION INSTRUCTIONS:
   - Do NOT include these props from the reference: [${options.excludedProps.join(", ")}].
      ...
      // TEXT style
      const enhancedPrompt = await enhanceStyleDescription(
...
- ${isFood ? "Rotten food, unappetizing colors." : "Broken glass, smudged labels, warped text, wrong logo spelling."}
  `;
    ...
    if (
      options.backgroundMode === BackgroundMode.CUSTOM &&
    ...
    if (rawResponse.candidates?.[0]?.content?.parts) {
      for (const part of rawResponse.candidates[0].content.parts) {
        if (part.inlineData?.data) {
          ...
          const result: EnhanceImageResponse = {
    ...
    if (imageTask) {
      await imageTask.update({ status: TaskStatus.FAILED });
    }
    return res
      .status(500)
 ...
 */
export const geminiProxy = async (req: AuthRequest, res: Response) => {
  try {
